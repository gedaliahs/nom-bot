import { Client, GatewayIntentBits, Partials, Events, SlashCommandBuilder, REST, Routes, EmbedBuilder, PermissionFlagsBits } from "discord.js";
import { getRandomResponse, initResponses } from "./responses.js";
import { bumpUserNom, incrTotalNom, getCounter, setNextDrop, getSetting, upsertSetting, topToday, topAll, resetMarathon, addWin, topWinners } from "./db.js";
import { nomTranslate } from "./translator.js";
import { seasonalTag } from "./seasonal.js";
import { maybeRunRareEvent } from "./rareEvents.js"; // keep your rare events file from earlier
...
const commands = [
  new SlashCommandBuilder().setName("stats").setDescription("Show nom stats (global/today/you)."),
  new SlashCommandBuilder().setName("leaderboard").setDescription("Show nom leaderboard (today & all-time)."),
  new SlashCommandBuilder().setName("setchannel").setDescription("Set the announcements/channel for nom posts.")
    .addChannelOption(o => o.setName("channel").setDescription("Target channel").setRequired(true))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
  new SlashCommandBuilder().setName("marathon").setDescription("Control Nom Marathon.")
    .addStringOption(o => o.setName("action").setDescription("start/stop").setRequired(true).addChoices(
      {name:"start", value:"start"}, {name:"stop", value:"stop"}
    )),
  new SlashCommandBuilder().setName("countdown").setDescription("Tease a golden drop after X minutes.")
    .addIntegerOption(o => o.setName("minutes").setDescription("Minutes (1-120)").setRequired(true)),
  new SlashCommandBuilder().setName("translate").setDescription("Translate text to actually good nom.")
...
client.once(Events.ClientReady, async (c) => {
  ...
  // Register global commands (simple)
...
function maybeNomParty(message) {
  ...
  if (pruned.length >= partyThreshold) {
    ...
    try { message.react("🍪"); message.react("🔥"); message.react("💫"); } catch {}
  }
}

// Mini-game drop every 30–40 noms
async function maybeDropTask(message, totalGuildNom) {
  const row = getCounter.get(message.guild.id);
  if (!row) return;
  const remaining = row.next_drop_at - (totalGuildNom % row.next_drop_at === 0 ? row.next_drop_at : (totalGuildNom % row.next_drop_at));
  // Simpler: trigger when total % next_drop_at === 0
  if (totalGuildNom % row.next_drop_at !== 0) return;

  // choose a task: react race or code race
  ...
  if (kind === "react") {
    ...
    try { await sent.react(emoji); } catch {}
    activeTasks.set(message.guild.id, { messageId: sent.id, type: "react", emoji, expiresAt, channelId: message.channel.id });
  } else {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({text:`Time limit: ${Math.round(ttl/1000)}s • ${seasonalTag()}`});
    const sent = await message.channel.send({ embeds: [embed] });
    activeTasks.set(message.guild.id, { messageId: sent.id, type: "code", answer: code, expiresAt, channelId: message.channel.id });
  }
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (!task) return;
  if (Date.now() > task.expiresAt) { activeTasks.delete(message.guild.id); return; }
  if (task.type === "code" && message.channel.id === task.channelId) {
    if (message.content.trim() === task.answer) {
      ...
      addWin.run(message.guild.id, message.author.id);
...
client.on(Events.MessageReactionAdd, async (reaction, user) => {
  ...
  if (reaction.emoji.name === task.emoji) {
    activeTasks.delete(message.guild.id);
    addWin.run(message.guild.id, user.id);
...
client.on(Events.InteractionCreate, async (itx) => {
  ...
  if (name === "stats") {
    ...
    const topT = topToday.all(itx.guildId, today);
    const topA = topAll.all(itx.guildId);
    const embed = new EmbedBuilder()
      .setTitle("📊 Nom Stats")
      .addFields(
        { name: "Today", value: topT.length ? topT.map((r,i)=>`${i+1}. <@${r.user_id}> — ${r.daily_nom}`).join("\n") : "No data" },
        { name: "All-time", value: topA.length ? topA.map((r,i)=>`${i+1}. <@${r.user_id}> — ${r.total_nom}`).join("\n") : "No data" }
      )
      .setColor(0x3B4756)
  ...
  if (name === "leaderboard") {
    const winners = topWinners.all(itx.guildId);
    const value = winners.length ? winners.map((r,i)=>`${i+1}. <@${r.user_id}> — ${r.wins} wins`).join("\n") : "No wins yet.";
    const embed = new EmbedBuilder().setTitle("🏆 Mini-Game Winners").setDescription(value).setColor(0xBFD7EA);
    await itx.reply({ embeds: [embed] });
  }

  if (name === "setchannel") {
    const ch = itx.options.getChannel("channel", true);
    upsertSetting.run(itx.guildId, ch.id);
  ...
  if (name === "marathon") {
    ...
    await itx.reply(start ? "🏁 **Nom Marathon STARTED!** Type 'nom' to contribute." : "🏁 **Nom Marathon STOPPED.** Good run!");
  }

  if (name === "countdown") {
    const minutes = itx.options.getInteger("minutes", true);
    if (minutes < 1 || minutes > 120) return itx.reply({ content: "Pick 1–120 minutes.", ephemeral: true });
    await itx.reply(`⏳ Golden Nom arriving in **${minutes}** minutes…`);
    setTimeout(async () => {
      const setting = getSetting.get(itx.guildId);
      const ch = setting ? await itx.guild.channels.fetch(setting.announce_channel_id).catch(()=>null) : itx.channel;
